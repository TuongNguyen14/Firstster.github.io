...
module.exports.GetUserByUsernamePassword = function GetUserByUsernamePassword(inusername, inpassword, callback) {
  ...
  UserModel.findOne({ username: inusername }, function (err, user) {
    ...
    if (user != null) {
      ...
      UserModel.find({ email: user.Email }, function (err, user) {
        if (user != null) {
          ...
          UserModel.create({ username: user.Username, password: user.Password, fullname: user.Fullname, email: user.Email, type: 1 },
            function (err, data) {
              if (err) callback(err);
              else callback();
            }
...
module.exports.UpdateUserSession = function UpdateUserSession(sessionId, userId, callback) {
  UserModel.findByIdAndUpdate(userId, { sessionId: sessionId }, function (err) {
    if (err) callback(err);
  });
};
